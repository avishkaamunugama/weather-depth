...
class vKittiDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        input_image = cv2.resize(image_rgb,(configs.IMAGE_SIZE))

        depth_name = os.path.join(self.root_dir,sample[1])
        depth_raw = cv2.imread(depth_name, cv2.IMREAD_ANYCOLOR | cv2.IMREAD_ANYDEPTH).astype(np.float32)/255
        depth_image = cv2.resize(depth_raw,(configs.IMAGE_SIZE))
...
def upscale_depth_img(depth_img, scale_factor=2):
    depth_img=depth_img.reshape(configs.IMAGE_SIZE[1]//2, configs.IMAGE_SIZE[0]//2)
...
def evaluate_model(model,eval_dataloader):
    ...
    preds = np.zeros((configs.IMAGE_SIZE[1], configs.IMAGE_SIZE[0], len(eval_dataloader.dataset)), dtype=np.float32)
    labels = np.zeros((configs.IMAGE_SIZE[1], configs.IMAGE_SIZE[0], len(eval_dataloader.dataset)), dtype=np.float32)

    # Starting evaluation ...
    for i,sample_test  in enumerate (eval_dataloader):
        img = torch.autograd.Variable(sample_test['image'].cuda())
        label = torch.autograd.Variable(sample_test['depth'])
